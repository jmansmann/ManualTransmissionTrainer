...
public abstract class SupervisedLearner {
	...
	public abstract Node train(Matrix features, Matrix labels) throws Exception;
	...
	public List<Double> makePredictions(Matrix features, Matrix labels, Matrix confusion) throws Exception {
		...
		List<Double> retPredictions = new ArrayList<Double>();
		if (labelValues == 0) {
			...
			for(int i = 0; i < features.rows(); i++)
			{
				...
				retPredictions.add(prediction[0]);
